using System;
using ModbusAscMasterLib;
using NModbus;

namespace ModbusAscMasterApp
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("Modbus ASCII Master App");
            
            string portName = args.Length > 0 ? args[0] : "COM3";
            int baudRate = 9600;
            byte slaveId = 1;

            using (var master = new ModbusAscMaster(portName, baudRate))
            {
                // 注册日志事件
                ...
                catch (Exception ex)
                {
                    ...
                    Console.WriteLine("Note: For ASCII test, ensure you have a virtual serial port pair (COM1 <-> COM2).");
                }
            }
