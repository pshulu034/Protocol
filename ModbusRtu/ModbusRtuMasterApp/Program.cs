using System;
using ModbusRtuMasterLib;
using NModbus;

namespace ModbusRtuMasterApp
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("Modbus RTU Master App");
            
            string portName = args.Length > 0 ? args[0] : "COM3";
            int baudRate = 9600;
            byte slaveId = 1;

            using (var master = new ModbusRtuMaster(portName, baudRate))
            {
                // 注册日志事件
                ...
                catch (Exception ex)
                {
                    ...
                    Console.WriteLine("Note: For RTU test, ensure you have a virtual serial port pair (COM1 <-> COM2).");
                }
            }
